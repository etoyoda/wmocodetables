...
* `GRIB2/GRIB2_CodeFlag`{usc}4{usc}2{usc}__d{usc}k__``{usc}CodeTable{usc}en.csv`` - 符号表4.2分野__d__カテゴリ__k__
...
* `BUFR4/BUFRCREX_TableB_en`{usc}__k__``.csv`` - BUFR/CREX表B第 _k_ クラス
...
* `BUFR4/BUFR_TableD_en`{usc}__k__``.csv`` - BUFR表D第 _k_ クラス
* `BUFR4/CREX_TableD_en`{usc}__k__``.csv`` - CREX表D第 _k_ クラス
