...
結局、CodeFlag, Value, およびMeaningParameterDescription_en+Note_enの
3列が通報式の文書に  
書かれる。
...
CodeFlag_table.csvには分野番号__d__ごとに別の行が与えられている。
通報式(WMO Codes)の文書では、  
符号表4.1は__d__毎に別の表、符号表4.2は__(d,k)__毎に別の表が組版されており、それぞれに注がついている。
...
指示する。
<<Status,Statusは表示に反映しない(後述)>>。
結局 OctetNo, OctetCount, Contents_en, Note_en が
表示されることになる。
...
これらを比較すると、表本体のCSVファイルが
存在するのに、
notes/Template_table.csvにエントリが欠けているものがあるので、
Title_en列を用いるべきである。
