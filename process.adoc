...
通報式(WMO Codes)の文書では、  
符号表4.1は _d_ 毎に別の表、符号表4.2は _(d,k)_ 毎に別の表が組版されており、それぞれに注がついている。

比較すると本表に対応する行が欠けている _s.n.d.k_ 値が存在するので、
本表は注釈番号を解釈するためだけに用いるべきである。
...
比較すると本表に対応する行が欠けている _s.t_ 値が存在するので、
本表は注釈番号を解釈するためだけに用いるべきである。
...
==== 注釈CSVの構造
`BUFR4/notes/BUFRCREX_CodeFlag_notes.csv` の構造は<<GCFNN,GRIB2符号表の注釈CSV>>と同様。
...
ただし 0 20 004/0 20 005 および 0 23 008/0 23 009 については
2個の要素記述子を本表では1行で書かれている。
これらの要素記述子には表全体への注釈がついていないので、
その解釈を誤っても問題はない。

比較すると本表に対応する行が欠けているFXY値が存在するので、
本表は注釈番号を解釈するためだけに用いるべきである。
