...
このうち第1,3,4,5,及び7節は
**テンプレート**番号(0..65535)によって異なる構造を持つ。
...
BUFR/CREX 通報式では、報文に記述されるデータの構造を「記述子」と呼ばれる整数の列で定義する。この記述子の体系は、用途に応じて 表A〜D の4種類の表で定義されている。
まず *表A* は、データ要素を大まかな分類に分けるための「クラス一覧」である。各種類の観測量（気温、風、放射、海洋など）がどのクラスに属するかを示し、要素記述子の体系全体を俯瞰する役割を担う。
次に *表B* は、実際の観測・予報資料の要素を定義する「要素記述子」を定義する表で、クラス毎に各要素の名称、単位、尺度、ビット幅といった属性を規定する。
*表C* は、データの読み取りに特別な操作を加えるための表で、記述子列の意味や解釈方法を変える各種の「操作記述子」を定義する。表Bだけでは表現できない数値の範囲拡大や付加的な情報などが表現される。
最後に *表D* は、複数の記述子を組み合わせてひとまとまりの構造として扱うための「集約記述子」を定義します。よく使われる記述子のセットを簡潔に表現でき、観測データ構造の記述を大幅に短縮する役割がある。
...
なお、CREX通報式はBUFRへの移行進展に伴い2024年に改正が凍結された
(Rec.8 INFCOM-3, Res. 17 EC-78)。
このためWMOが提供するCSVデータもCREXの改正に備えた注釈データを
欠いている。
...
上記でいう「表」は通報式の法規的な論理上の概念である。以下本文書では論理表（logical table）と呼ぶ。
